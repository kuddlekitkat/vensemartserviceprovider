...
import 'package:vensemartserviceprovider/ServiceIntroScreen.dart';
import 'package:vensemartserviceprovider/ServiceLocationScreen.dart';
import 'package:vensemartserviceprovider/model/accept_reject_booking.dart';
import 'package:vensemartserviceprovider/model/bank_plans.dart';
import 'package:vensemartserviceprovider/model/current_plan_details_model.dart';
...
class ProviderServices extends ChangeNotifier {
  ...
  CurrentPlanDetailsModel? get currentPlanDetailsModel => _currentPlanDetailsModel;
  CurrentPlanDetailsModel? _currentPlanDetailsModel;
  ...
  BankProviderPlans? get bankProviderPlans => _bankProviderPlans;
  BankProviderPlans? _bankProviderPlans;
  ...
  CanceledRequestModel? get canceledRequestModel => _canceledRequestModel;
  CanceledRequestModel? _canceledRequestModel;
  ...
  void bank_plans() async {
    try {
      _isLoading = true;
      Response? response = await authRepo.bankPlans();
      if (response != null && response.statusCode == 200) {
        _bankProviderPlans = BankProviderPlans.fromJson(response.data);
        _isLoading = false;
      }
      notifyListeners();
    } catch (e, str) {
  ...
  void canceledRequests() async {
    try {
      ...
      if (response != null && response.statusCode == 200) {
        _canceledRequestModel = CanceledRequestModel.fromJson(response.data);
        _isLoading = false;
      }
      notifyListeners();
    } catch (e, str) {
  ...
  //   try {
  //     _isLoading = true;
  //     Response? response = await authRepo.com();
  //     if (response != null && response.statusCode == 200) {
  //       _upcomingRequest = UpcomingRequest.fromJson(response.data);
  //       _isLoading = false;
  //     }
